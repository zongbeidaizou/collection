...
class SmsHistoryPresenter extends BasePagePresenter<SmsHistoryPageMvpView> {
  ...
  Future<void> index(int borrowId, int page, bool isShowDialog,
      {String keyword = ''}) async {
    final String? cacheData = await Cache().checkCache('sms_history_$borrowId');
    if (cacheData == null) {
      await requestNetwork<HKContactSmsEntity>(Method.get,
          url: HttpApi.smsHistory,
          queryParameters: {'page': page, 'borrow_id': borrowId},
          onSuccess: (data) async {
        if (data != null) {
          Cache().cacheData('sms_history_$borrowId', data.toString(), 3600 * 8);
      ...
      view.setList(HKContactSmsEntity.fromJson(
